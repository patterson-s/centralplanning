...
st.set_page_config(page_title="Notes", page_icon="📝", layout="centered")
st.title("📝 Notes")
...
tab_voice, tab_camera = st.tabs(["🎙️ Voice Notes", "📷 Camera"])
...
with tab_camera:
    ...
    if image:
        ...
        photo_filename = f"photo_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.jpg"
        try:
            conn = psycopg2.connect(st.secrets["NEON_DATABASE_URL"])
            cur = conn.cursor()
            cur.execute(
                """
                INSERT INTO ocr_photos (context_text, image_data, image_filename)
                VALUES (%s, %s, %s)
                """,
                (
                    st.session_state.get("context_text", ""),
            ...
            cur.close()
            conn.close()
            st.success("Photo saved for OCR.")
        except Exception as e:
            st.error(f"Upload failed: {e}")
